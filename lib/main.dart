...
const String _userLatStorageKey = 'hapke_user_lat';
const String _userLngStorageKey = 'hapke_user_lng';
...
Uri _restaurantsUri({double? lat, double? lng}) {
...
// Geen hardcoded gerechtfoto's meer; we gebruiken API / restaurantfoto
...
class _HapkeAppState extends State<HapkeApp> {
  ...
  Future<void> _restoreLocation() async {
    try {
      final latRaw = await _secureStorage.read(key: _userLatStorageKey);
      final lngRaw = await _secureStorage.read(key: _userLngStorageKey);
      final lat = double.tryParse((latRaw ?? '').replaceAll(',', '.'));
      final lng = double.tryParse((lngRaw ?? '').replaceAll(',', '.'));
  ...
  Future<void> _saveLocation(double lat, double lng) async {
    ...
    try {
      await _secureStorage.write(key: _userLatStorageKey, value: '$lat');
      await _secureStorage.write(key: _userLngStorageKey, value: '$lng');
    } catch (e) {
  ...
  Future<void> _loadRestaurants() async {
    ...
    try {
      ...
      final res = await apiClient.get(
        _restaurantsUri(lat: lat, lng: lng),
      );
      if (res.statusCode == 200) {
  ...
  Widget _buildHome() {
    return _RootTabs(
      ...
      locationRequired: _needsLocation,
      userLat: _userLat,
      userLng: _userLng,
    ...
    return MaterialApp(
      ...
      theme: ThemeData(
        ...
        colorScheme: ColorScheme.fromSeed(
          seedColor: const Color(0xFF2AAAB3),
        ...
        appBarTheme: const AppBarTheme(
          centerTitle: true,
          elevation: 0,
          backgroundColor: Color(0xFF2AAAB3),
        ...
        outlinedButtonTheme: OutlinedButtonThemeData(
          style: OutlinedButton.styleFrom(
            foregroundColor: const Color(0xFF2AAAB3),
            side: const BorderSide(color: Color(0xFF2AAAB3)),
          ),
        ),
        ...
        chipTheme: ChipThemeData(
          ...
          secondarySelectedColor: const Color(0xFF2AAAB3),
        ),
...
class _RootTabsState extends State<_RootTabs> {
  ...
  Future<void> _checkout(BuildContext context) async {
    ...
    if (widget.userLat == null || widget.userLng == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Vul je locatie in om te bestellen')),
      );
    ...
    final result = await Navigator.of(context).push<Map<String, dynamic>>(
      MaterialPageRoute(
        builder: (_) => PaymentMethodPage(
          ...
          deliveryLat: widget.userLat,
          deliveryLng: widget.userLng,
        ),
      ),
    );
  ...
  Future<Map<String, dynamic>?> _completeOrder(String paymentId) async {
    ...
    try {
      ...
      if (res.statusCode == 400 && mounted) {
        setState(() {
          _error =
              'Dit restaurant bezorgt niet op jouw locatie of locatie ontbreekt.';
        });
      }
      if (mounted) {
        setState(() {
      ...
      if (res.statusCode == 200) {
        ...
        setState(() {
          ...
          final alreadyVerified =
              (data != null && data['alreadyVerified'] == true);
...
class _VideosTabState extends State<VideosTab> {
  ...
  Future<void> _loadAllVideos() async {
    ...
    if (widget.userLat != null && widget.userLng != null) {
      try {
        final res = await apiClient.get(
          _restaurantsUri(lat: widget.userLat, lng: widget.userLng),
          headers: {'Accept': 'application/json'},
        );
        ...
        if (res.statusCode == 200) {
          ...
          debugPrint(
            '[VIDEOS] restaurants error: ${res.statusCode} ${res.body}',
          );
        }
      } catch (e) {
      ...
      debugPrint('[VIDEOS] skip /restaurants (geen locatie)');
    }
    // Dedup
    final seen = <String>{};
  ...
  Future<void> _placeOrderFromVideo({
    ...
    if (widget.userLat == null || widget.userLng == null) {
      if (mounted) {
        setState(() {
          _orderSuccess = 'Vul je locatie in om te bestellen';
        });
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Vul je locatie in om te bestellen')),
        );
